...
title: "Exercise 1"
author: "Ignatova S.I."
date: '28 С„РµРІСЂР°Р»СЏ 2019 Рі '
output:
...
Функция  взаимосвязи: f(x)=8+3.5*sin((x-30)/9)
Обьем обучающей выборки 85%.
```{r }
#  Генерируем данные 
...
y.model.train <- predict(mod, data.frame(x.train))$y
y.model.test <- predict(mod, data.frame(x.test))$y
...
for (i in 2:max.df) {
  ...
  y.model.train <- predict(mod, data.frame(x.train))$y[,1]
  y.model.test <- predict(mod, data.frame(x.test))$y[,1]
...
по графику видим, что достаточно высокая точность модели достигается в модели с df=8
```{r}
#  График 3: Лучшая модель (компромисс между гибкостью и точностью) ############
...
#Увеличив объем выборки в 10 раз, мы заметили, что MSE изменилась не сильно, однако для тестовой выборки второй модели она стала чуть меньше 